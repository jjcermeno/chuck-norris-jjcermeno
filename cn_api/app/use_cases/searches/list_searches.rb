module Searches
  class ListSearches
    ...
    def calculate_pages
      ...
      min_page_number     = 1
      max_page_size       = 100
      page_size           = params['page_size'].present? ? params['page_size'].to_i : default_page_size
      page_size           = max_page_size if page_size > max_page_size
      page_size           = min_page_size if page_size.negative?
      page_number         = params['page_number'].present? ? params['page_number'].to_i : default_page_number
    ...
    def get_searches
      @meta[:totalPages] = (searches_repository.total_searches / per_page) + 1
      @meta[:page] = page
